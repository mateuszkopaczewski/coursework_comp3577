...
class Test(object):
  ...
  def __init__(self, compiler, submitted_file, benchmark_file = ""):
    """
  ...
  def __get_submitted_executable_name(self):
    return self.submitted_file + "-" + self.compiler

  def __get_benchmark_executable_name(self):
    return "benchmark.out"


  def compile(self,compiler_call="CXX=\"g++\" CXXFLAGS=\"-O3\""):
    """
    ...
    success = True
    try:
      invocation = compiler_call + " make " + self.__get_submitted_executable_name()
      print(invocation)
